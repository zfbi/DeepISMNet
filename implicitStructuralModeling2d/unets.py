...
from modules.mobile_netv3 import Block, hswish, SeModule
from modules.hlconv import *

class conv_block(nn.Module):
    def __init__(self, ch_in, ch_out, 
                 kernel_size=3, stride=1, batch_norm=nn.BatchNorm2d):
        ...
        self.conv = nn.Sequential(
            ...
            nn.Conv2d(ch_out, ch_out, kernel_size=kernel_size,stride=1,
                      padding=kernel_size//2,bias=True),
            batch_norm(ch_out),
            nn.ReLU(inplace=True)
        )
...
class up_conv_dep(nn.Module):
    def __init__(self,ch_in,ch_out,use_hswish=False):
        super(up_conv_dep,self).__init__()
        hlConv = hlconv["dep_sep_conv"] 
